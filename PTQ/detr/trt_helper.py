...
class TrtNetworkHelper():
    ...
    def addLayerNorm(self, x, gamma, beta, layer_name=None, precision=None):
        ...
        # Print data type of gamma and beta tensors
        # print("Data type of x tensor:", trt.nptype(x.dtype))
        # print("Data type of gamma tensor:", trt.nptype(gamma_dtype))
        # print("Data type of beta tensor:", trt.nptype(beta_dtype))
        # print(f"x.shape:{x.shape}")
        # print(f"gamma.shape:{gamma.shape}")
        # print(f"beta.shape:{beta.shape}")
        # pluginLayer = self.network.add_plugin_v2([x], getLayerNormPlugin(gamma_field, beta_field))
        pluginLayer = self.network.add_plugin_v2([x, gamma, beta], getLayerNormPlugin())
        # print(f"add layernorm success!")
    ...
    def addLinear(self, x, weight, bias, layer_name=None, precision=None):
        ...
        constant_layer = self.network.add_constant(weight.shape, trt.Weights(weight))
        x_mul = self.network.add_matrix_multiply(x, 
    ...
    def addScale(
        ...
        trt_layer = self.network.add_scale(x,
