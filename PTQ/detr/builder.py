...
handle = ctypes.CDLL("baseLayerNormPlugin/LayerNorm.so", mode=ctypes.RTLD_GLOBAL)
if not handle:
...
def self_attention_layer(network_helper, prefix, config, weights_dict, input_tensor, imask):
    # network_helper.markOutput(input_tensor)
    ...
    network_helper.markOutput(scores)
...
def transformer_layer(network_helper, prefix, config, weights_dict, input_tensor, imask):
    ...
    # network_helper.markOutput(attention_output)

    # BertIntermediate
    # print(f"weights_dict:{weights_dict}")
    intermediate_w = weights_dict[prefix + "intermediate_dense_kernel"]
    # print(weights_dict[prefix + "intermediate_dense_kernel"])
    ...
    # network_helper.markOutput(layer_output)
...
def build_engine(workspace_size, config, weights_dict, vocab_file, calibrationCacheFile, calib_num):
    ...
    if config.use_int8:
        ...
        calibrator = BertCalibrator("calibrator_data.txt", "../../chapter_five/bert-base-uncased", calibrationCacheFile, 1, max_seq_length, 1000)
    ...
    for i in range(network_helper.network.num_layers):
        layer = network_helper.network.get_layer(i)
        if "intermediate_dense" in layer.name:
...
def test_text(infer_helper, BERT_PATH):
    ...
    print(f"top_10.type:{type(top_10)}")
...
def main():
    ...
    if args.onnx != None:
        ...
        # print(f"weights_dict:{type(weights_dict)}")
    ...
    engine = build_engine(args.workspace_size, config, weights_dict, args.vocab_file, calib_cache, args.calib_num)
    serialized_engine = engine.serialize()
    TRT_LOGGER.log(TRT_LOGGER.INFO, "Saving Engine to {:}".format(args.output))
    with open(args.output, "wb") as fout:
        fout.write(serialized_engine)
