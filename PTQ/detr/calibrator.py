...
class BertCalibrator(trt.IInt8LegacyCalibrator):
    def __init__(self, data_txt, bert_path, cache_file, batch_size, max_seq_length, num_inputs):
        ...
        def text2inputs(tokenizer, text):
            encode_input = tokenizer.encode_plus(text, return_tensors = "pt")
            input_ids = encode_input['input_ids'].int().detach().numpy()
            token_type_ids = encode_input['token_type_ids'].int().detach().numpy()
            seq_len = encode_input['input_ids'].shape[1]
        ...
        if num_inputs > len(self.input_ids_list):
            print(f"self.input_ids_list:{self.input_ids_list}")
    ...
    def get_batch(self, names):
        ...
        print(f"input_ids.ravel():{input_ids.shape}")
...
if __name__ == '__main__':
    ...
    bert_path = "../../chapter_five/bert-base-uncased"
